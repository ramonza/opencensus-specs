...
When an implicit propagated Context is used implementation MUST use scoped objects to 
...
When an explicit propagated Context is used implementation MUST create a new Context when a Span 
is attached (immutable Context):
* When attach/detach an already created Span the API MAY be called `WithSpan`.
* When attach/detach at the creation time the API MAY be called `StartSpan` or `StartScopedSpan`.
...
### Why to support Spans are not always attached to the Context?
